...
def plot_episode_stats(stats, title, smoothing_window=10, noshow=False):
	...
	if title == "Training":
		...
		for i, stat in enumerate(stats):
			name = 'new_results/' + 'q_values.txt' + str(i)
		...
		for i, stat in enumerate(stats):
			name = 'new_results/' + 'rewards' + str(i) + '.txt'
			...
			name = 'new_results/' + 'rewards_mean' + str(i) + '.txt'
		...
		for i, stat in enumerate(stats):
			...
			fig4.savefig('new_results/' + 'percentile_reward.png', bbox_inches='tight')
		...
		#for i, stat in enumerate(stats):
		np.savetxt('new_results/' + 'rew.txt',stats.episode_rewards, delimiter=',', fmt='%.8f')
