...
class CriticNetwork_Q():
	...
	def _build_model(self):
		...
		# Output layer will have linear activation with "activation_fn=None"
		# Name changed to predictions
		self.action_values = tf.contrib.layers.fully_connected(self.fc2, action_space_size, activation_fn=None)
		...
		gather_indices = tf.range(batch_size) * tf.shape(self.action_values)[1] + self.actions
		...
		self.losses = tf.squared_difference(self.target, self.action_value)
		self.loss = tf.reduce_mean(self.losses)
	...
	def predict(self, sess, states):
		...
		Returns:
			The prediction of the output tensor.
		"""
		predictions = sess.run(self.action_values, { self.state: states})
	...
	def update(self, sess, states, actions, targets):
		...
		"""
		# Get input (states), labels (actions), and predictions (targets) 
		feed_dict = { self.state: states, self.actions: actions, self.target: targets }
...
class ActorNetwork():
	...
	"""def epsilon_greedy(epsilon):
		...
		probabilities = np.ones(action_space_size) * epsilon / action_space_size
	...
	def decaying_epsilon_greedy(epsilon, schedule):
		# Implement this!

		decayed_epsilon = schedule(epsilon)

		# Which arm(bandit)?
		return epsilon_greedy(decayed_epsilon)"""


def pendulum(sess, env, actor, critic_v, critic_q, num_episodes, max_time_per_episode, discount_factor=0.99, epsilon=0.1):
	...
	for i_episode in range(num_episodes):
		...
		for t in itertools.count():
			...
			critic_q.update(sess, [state], [action], [td_target_q])
...
if __name__ == "__main__":
	env = PendulumEnv()
	actor = ActorNetwork()
	critic_v = CriticNetwork_V()
	critic_q = CriticNetwork_Q()

	"""epsilon = 0.7
			
				def schedule(epsilon, time_step):
					# Simple exponential decay 
					decaying_factor = 100000
			
					return epsilon * np.exp(-((time_step + 1)/decaying_factor))
	...
	stats = pendulum(sess, env, actor, critic_v, critic_q, 200, 1000)
